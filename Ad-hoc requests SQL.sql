...
select * from dbo.fact_gross_price;
select * from dbo.fact_manufacturing_cost;
select * from dbo.fact_pre_invoice_deductions;
select * from dbo.fact_sales_monthly;

select fiscal_year from dbo.fact_sales_monthly;
select distinct region from dbo.dim_customer;
...
percentage_chg*/
with cte1 as(
select count(distinct product_code) AS unique_products_2021 from dbo.fact_gross_price where fiscal_year=2021),

cte2 as(
select count(distinct product_code) AS unique_products_2020 from dbo.fact_gross_price where fiscal_year=2020
),
cte3 as(
...
select segment,count(distinct product_code) as product_count from dbo.dim_product group by segment
...
with cte1 as(
select a.product_code,a.segment,b.fiscal_year from dim_product as a join fact_gross_price as b on 
a.product_code=b.product_code),

--counted product from the year 2021 by segment
cte2 as(
select segment,count(distinct product_code) as product_count2021 
from cte1 where fiscal_year=2021 group by segment),

--counted product from the year 2020 by segment
cte3 as(
select segment,count(distinct product_code) as product_count2020
...
cte4 as(
select cte2.segment,cte2.product_count2021,cte3.product_count2020,
(cte2.product_count2021-cte3.product_count2020) AS differences from 
...
from dim_product as a join fact_manufacturing_cost as b on
a.product_code=b.product_code),
...
with cte1 as(
select a.customer_code,a.customer,a.market,b.fiscal_year,b.pre_invoice_discount_pct from
dim_customer as a join fact_pre_invoice_deductions as b on a.customer_code=b.customer_code),

cte2 as(
select customer_code,customer,avg(pre_invoice_discount_pct) as high_envoice,market from
cte1 where fiscal_year=2021 group by customer_code,customer,market)

select top 5 customer_code,customer,high_envoice from cte2 
where market='India' order by high_envoice desc;
...
cte2 as(
select a.date,a.customer_code,a.sold_quantity,a.product_code,a.fiscal_year from 
 fact_sales_monthly as a join cte1 as b on a.customer_code=b.customer_code),

cte3 as(
select YEAR(date) as [Year],DATENAME(M,date) as [Month],sold_quantity,gross_price from cte2 as a join fact_gross_price as b
on a.product_code=b.product_code)

select [Year],[Month],
SUM((gross_price*sold_quantity)) as Gross_sales_Amount from cte3 group by [Month],[Year]
...
end as qtble from fact_sales_monthly where fiscal_year=2020 order by date;
-----------------------------------------------------------------------
...
end as [Quarter],sold_quantity from fact_sales_monthly 
where fiscal_year=2020)

select [Quarter],SUM(sold_quantity) as total_sold_quantity from 
cte group by [Quarter] order by total_sold_quantity desc;
...
--create view-you can not place order by clause under view/place it on theouter query
alter view [qt_sold] as
with cte as(
select distinct [date],product_code,
case when date between '2019-09-01' and '2019-11-01' then 'Q1'
     when date between '2019-11-01' and '2020-02-01' then 'Q2'
	 when date between '2020-02-01' and '2020-05-01' then 'Q3'
	 when date between '2020-05-01' and '2020-08-01' then 'Q4'
...
select [Quarter],product_code,SUM(sold_quantity) as total_sold_quantity from 
cte group by [Quarter],product_code;

select * from [qt_sold] order by total_sold_quantity desc;
...
--% contribution= SUM((gross_price*sold_quantity= gross_sales_mln)/sum(gross_sales_mln) *100
...
cte2 as(
      select a.*,gross_price from cte1 as a join fact_gross_price as b
...
cte3 as(
       select channel,
       CAST(SUM((gross_price*sold_quantity))/1000000 AS DECIMAL(10,2))as gross_sales_mln 
	   from cte2 where fiscal_year=2021 group by channel
)

--show the channel wise gross sales in million unit and percentage of contribution- using subquery
select channel,gross_sales_mln,ROUND(CAST((gross_sales_mln/contribute)*100 AS DECIMAL(10,2)),2) as percentage
from cte3,(select sum(gross_sales_mln) as contribute from cte3) as cont_table
